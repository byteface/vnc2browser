...
class RFBClient(Protocol):
    ...
    def _handleInitial(self):
        print('_handleInitial')
        self._packet = [p.decode('latin-1') for p in self._packet]
        buffer = ''.join(self._packet)
        # buffer = self._packet.decode('utf-8')
        print(buffer)
        print('\n' in buffer)
    ...
    def _handleServerInit(self, block):
        # print(len(block))
        # block = [b.encode() for b in block]
        (self.width, self.height, pixformat, namelen) = unpack("!HH16sI", bytes(block,'latin-1'))
    ...
    def _handleConnection(self, block):
        (msgid,) = unpack("!B", bytes(block,'latin-1'))
        if msgid == 0:
    ...
    def _handleFramebufferUpdate(self, block):
        (self.rectangles,) = unpack("!xH", bytes(block, 'latin-1'))
        self.rectanglePos = []
        self.beginUpdate()
        self._doConnection()
    ...
    def _handleRectangle(self, block):
        (x, y, width, height, encoding) = unpack("!HHHHI", bytes(block,'latin-1'))
        if self.rectangles:
            self.rectangles -= 1
    ...
    def _handleDecodeHextile(self, block, bg, color, x, y, width, height, tx, ty):
        (subencoding,) = unpack("!B", bytes(block,'latin-1'))
        #calc tile size
        tw = th = 16
    ...
    def _handleExpected(self):
        ...
        # self._packet = [p.decode() for p in self._packet]
        newpacket = []
        for p in self._packet:
            if type(p) is bytes:
                p = p.decode('latin-1')
            newpacket.append(p)

        self._packet = newpacket
        ...
        if self._packet_len >= self._expected_len:
            ...
            while len(buffer) >= self._expected_len:
                ...
                block, buffer = buffer[:int(self._expected_len)], buffer[int(self._expected_len):]
